    ...
    <div id="holding-table" >
        <div class="table-wrapping">
            <el-table ref="multipleTable" :data="holdings" style="width: 100%; font-size: 10px;" @selection-change="handleSelectionChange" :stripe="true" :highlight-current-row="true" :default-sort = "{prop: 'nom', order: 'ascending'}" class="h-border">
                ...
                <el-table-column property="entreprises" label="ENTREPRISES" sortable width="117"><template scope="scope" ><div class="data-wrapper md-txt">{{scope.row.iE}}</div></template></el-table-column>
                <el-table-column property="contrats.length" label="CONTRAT" sortable width="100"><template scope="scope"><div class="data-wrapper pointer md-txt holHover">{{scope.row.iC}}</div></template></el-table-column>
                <el-table-column property="beneficiaire" label="NOMBRE D'ASSURES ET BENEFICIAIRES" width="230"><template scope="scope" ><div class="data-wrapper pointer md-txt holHover">{{scope.row.iA}}/{{scope.row.iB}}</div></template></el-table-column>
                <el-table-column property="totalCotisations" label="COTISATIONS" sortable width="115">
                <template slot-scope="scope">
                    <el-popover trigger="hover" placement="top">
                    <span>date: waiting info</span>
                    <div slot="reference" class="name-wrapper"><div class="data-wrapper pointer md-txt holHover">{{scope.row.iCo}}</div></div>
                    </el-popover>
                </template>
                </el-table-column>
                <el-table-column property="totalPrestations" label="PRESTATIONS" sortable width="115"><template scope="scope" >{{scope.row.iPr}}<div class="data-wrapper pointer md-txt holHover"></div></template></el-table-column>
